...
import { CheckCircle, Clock, MessageCircle, User, Tag, CreditCard } from "lucide-react";
...
function formatCurrency(n: number) {
  // assume reais. Ajuste conforme necessidade.
...
return (
  <div
    className="w-full p-6 md:p-8 bg-[var(--bg)] rounded-2xl text-[var(--text)] shadow-lg hover:shadow-[0_0_25px_-5px_var(--primary)/20]"
    role="region"
  ...
  >
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      {/* SIDEBAR -------------------------------------------------- */}
      <aside className="space-y-4" aria-label="Detalhes do serviço">
        <div
          className="border border-[var(--border)] rounded-2xl p-6 bg-[var(--bg-light)] shadow-lg hover:shadow-[0_0_25px_-5px_var(--primary)/20]"
          role="region"
          aria-label="Informações financeiras e prazo"
        >
          <p className="text-sm text-[var(--text-muted)]">Valor acordado</p>
          <p className="text-lg font-semibold">{formatCurrency(details.price)}</p>

          <p className="mt-4 text-sm text-[var(--text-muted)]">Prazo restante</p>
          <div className="flex items-center gap-2 text-[var(--text)] mt-1">
            <Clock size={14} aria-hidden /> <span>{details.deadline}</span>
          </div>

          <p className="mt-4 text-sm text-[var(--text-muted)]">Reputação</p>
          <p className="font-medium">{details.rating} ★</p>

          <p className="mt-4 text-sm text-[var(--text-muted)]">Pagamento</p>
          <div className="flex items-center gap-2 text-[var(--text)] mt-1">
            <CreditCard size={14} aria-hidden /> <span>{details.paymentMethod}</span>
          </div>
        </div>

        <div
          className="border border-[var(--border)] rounded-2xl p-4 bg-[var(--bg-light)] shadow-sm"
          aria-label="Documentos do serviço"
        >
          <p className="text-xs text-[var(--text-muted)] mb-2">Documentos</p>
          <p className="text-sm">Comprovantes enviados (3)</p>
        </div>
      </aside>
    </div>
  </div>
